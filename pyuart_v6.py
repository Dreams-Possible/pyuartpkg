...
class UartComuIns():
    ...
    # 检查包
    def _ckpkg(self,pkg: bytes) -> bool:
        print(f"buff_ckpkg={pkg.hex()}")
        # 检查包头包尾
    ...
    def _dcpkg(self,pkg: bytes) -> bytes:
        # 检查包头包尾
        if not self._ckpkg(pkg):
            print("bad packet header or tail")
    ...
    # 寻找包头
    def _find_head(self,buff: bytes) -> int:
        head_pos=0
        while True:
            try:
                # 寻找包头位置
                print(f"buff_find_head={buff.hex()}")
                head_pos = buff.index(self.HEAD, head_pos)
            except ValueError:
                # 无包头
                return None
                # 判断是否为转译
            if head_pos == 0 or buff[head_pos - 1] != self.ESCAPE:
                break
            else:
                head_pos += 1
        return head_pos
    ...
    # 寻找包尾
    def _find_tail(self,buff: bytes) -> int:
        tail_pos=0
        while True:
            try:
                # 寻找包尾位置
                tail_pos = buff.index(self.TAIL, tail_pos)
            except ValueError:
                # 无包尾
                return None
                # 判断是否为转译
            if buff[tail_pos - 1] != self.ESCAPE:
                break
            else:
                tail_pos += 1
        return tail_pos
    ...
    # 提取包
    def _parse_pkgs(self,buff: Union[bytes,bytearray]) -> list[bytes]:
        pkgs = []
        while True:
            # 寻找包头
            print(f"buff_parse_pkgs={buff.hex()}")
            head_pos=self._find_head(buff)
            # 无包头（非法包，清空缓冲区）
            if head_pos is None:
                print("no packet header")
                buff.clear()
                break
            # 清除包头前的异常数据
            if head_pos > 0:
                print(f"pos={head_pos}")
                print("clear abnormal data before packet header")
                del buff[:head_pos]
                head_pos = 0
            # 寻找包尾
            tail_pos=self._find_tail(buff)
            # 无包尾（可能是未接收完）
            if tail_pos is None:
                break
            # 取完整包
            pkg = bytes(buff[head_pos:tail_pos+1])
            # 从缓冲区删除该包
            del buff[:tail_pos+1]
            # 解包
            try:
                decoded = self._dcpkg(pkg)
            except:
                print("package data exception")
                return None
            pkgs.append(decoded)
            # 缓冲区已空退出
            if not buff:
                print("buff_parse_pkgs_empty")
                break
        return pkgs

    # 串口通信接收
    def recv(self,overtime=0) -> list[bytes]:
        # 接收
        data = self.uart.recv()
        # 检查是否有数据
        if data is None:
            return None
        print("Raw received data:", data.hex())
        # 存入缓冲区
        print(f"buff_recv_buff={self.recv_buff.hex()}")
        self.recv_buff.extend(data)
        # 提取包
        buff = self.recv_buff
        print(f"buff_recv={buff.hex()}")
        recv = []
        count = 0
        while True:
            recv=self._parse_pkgs(buff)
            if recv is None:
                if count < overtime:
                    recv=self._parse_pkgs(buff)
                    time.sleep(0.001)
